...
xlabel('COMPRIMENTO (m)')
ylabel('COMPRIMENTO (m)')
title(sprintf('Simulação no tempo: %d Vel X: %.2f Vel Y: %.2f Propriedade escalar: %.2f',j,uadv(j),vadv(j),cp(j)),'fontsize',12)
caxis([A B])
colorbar
print(strcat('fig_c_',num2str(100+j)),'-dpng','-r300');
if (j==1)||(j==ceil(timesim/2))||(j==timesim)
    print(strcat('plot_c_',num2str(j),'_cd_',num2str(mi),'_cp_',num2str(cp(j)),'_vx_',num2str(uadv(j)),'_vy_',num2str(vadv(j))),'-djpeg','-r600')
end
close(gcf)
%M(j) = getframe;
end

!convert -delay 60 -loop 0 fig_c_*.png dispersao_2_c.gif
!rm fig_c_*.png
...
print('plot_q_2_c','-dpng','-r600');
fprintf('\nQuestão 2 C: Simulação finalizada\n');
ls
else
