...
%uadv(1:timesim) = 0;%VELOCIDADE DE ADVECÇÃO EM X
%vadv(1:timesim) = 2.5;%VELOCIDADE DE ADVECÇÃO EM Y
%Propriedade escalar;
cp = 10;

uadv(1:timesim) = 0;%VELOCIDADE DE ADVECÇÃO EM X
vadv(1:timesim) = 4;%VELOCIDADE DE ADVECÇÃO EM Y
...
x=[0:deltay:alfa]; %DOMINIO ESPACIALvadv
...
%
%b_tr
%date,'_', num2str(hour(now)),'_', num2str(minute(now)),'_', num2str(floor(second(now))))
...
xlabel('COMPRIMENTO (m)')
ylabel('COMPRIMENTO (m)')
title(sprintf('Simulação no tempo: %d ',j),'fontsize',12)
caxis([A B])
colorbar
print(strcat('fig_a_',num2str(100+j)),'-dpng','-r300');
if (j==1)||(j==ceil(timesim/2))||(j==timesim)
    print(strcat('plot_a_',num2str(j),'_cd_',num2str(mi),'_cp_',num2str(cp),'_vx_',num2str(uadv(j)),'_vy_',num2str(vadv(j))),'-djpeg','-r600')
end
close(gcf)
%M(j) = getframe;
end 

!convert -delay 60 -loop 0 fig_a_*.png dispersao_2_a.gif
!rm fig_a_*.png
...
print('plot_q_2_a','-dpng','-r600');
fprintf('\nQuestão 2 A: Simulação finalizada\n');
ls
else
